...
- **History tab.** Every notice is saved there. It starts with ten sample notices.
...
## For developers
Edit `src/app.html`, `src/templates.js` (the four reference notices) or `src/samples.js` (the ten sample notices), then run `python3 build.py`. This rebuilds `index.html` and the desktop copy.
